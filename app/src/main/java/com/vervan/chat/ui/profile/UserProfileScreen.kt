...
fun UserProfileScreen(onBack: () -> Unit) {
    ...
    Scaffold(
        ...
        Column(Modifier.fillMaxSize().padding(padding).imePadding().padding(16.dp).verticalScroll(rememberScrollState())) {
            ...
            ProfileField("Preferred name", name, maxLength = ValidationLimits.USER_PREFERRED_NAME) { scope.launch { vm.setName(it) } }
            ProfileField("Occupation", occupation, "e.g. mobile engineer", maxLength = ValidationLimits.USER_OCCUPATION) { scope.launch { vm.setOccupation(it) } }
            ProfileField("Expertise level", expertise, "e.g. advanced", maxLength = ValidationLimits.USER_EXPERTISE) { scope.launch { vm.setExpertise(it) } }
            ChipInputField(
                items = interests.split(",").map { it.trim() }.filter { it.isNotEmpty() },
                onItemsChange = { scope.launch { vm.setInterests(it.joinToString(",")) } },
                label = "Interests",
            ...
            FlowRow(horizontalArrangement = Arrangement.spacedBy(4.dp)) {
                LanguageOptions.common.forEach { lang ->
                    FilterChip(
                        ...
                        onClick = { scope.launch { vm.toggleLanguage(lang, languages) } },
                        label = { Text(lang) }
                    )
                }
            }
            ...
            FlowRow(horizontalArrangement = Arrangement.spacedBy(4.dp)) {
                LanguageOptions.coding.forEach { lang ->
                    FilterChip(
                        ...
                        onClick = { scope.launch { vm.toggleCoding(lang, coding) } },
                        label = { Text(lang) }
                    )
                }
            }
            ...
            SingleChoiceSegmentedButtonRow {
                listOf("metric" to "Metric", "imperial" to "Imperial").forEachIndexed { i, (id, label) ->
                    SegmentedButton(
                        selected = units == id,
                        onClick = { scope.launch { vm.setUnits(id) } },
            ...
            ChipInputField(
                ...
                onItemsChange = { scope.launch { vm.setAvoid(it.joinToString(",")) } },
                label = "Topics to avoid",
                maxItemLength = ValidationLimits.USER_AVOID_TOPIC_ITEM,
                maxItemCount = ValidationLimits.USER_AVOID_TOPIC_COUNT,
                modifier = Modifier.padding(top = 12.dp)
            )
            ProfileField("Current goals", goals, "what are you working toward?", maxLength = ValidationLimits.USER_GOALS) { scope.launch { vm.setGoals(it) } }
        }
    }
}
