...
class UserProfileViewModel(app: VervanApp) : ViewModel() {
    ...
    val coding: StateFlow<Set<String>> = repo.userCodingLanguages.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptySet())
    ...
    fun toggleCoding(lang: String, current: Set<String>) { viewModelScope.launch { repo.setUserCodingLanguages(if (lang in current) current - lang else current + lang) } }
