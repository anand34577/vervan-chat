...
fun OnboardingScreen(onDone: () -> Unit, onImportModel: () -> Unit = {}) {
    ...
    val pages = buildList {
        ...
        add(OnboardPage("Offline tutorial", "• Type / to use slash commands and templates\n• Tap + to attach a photo, document, or voice recording\n• Tap a message to reveal its actions\n• Open More options for the context inspector and branch tree\n• Use the branch controls below a response when alternatives exist", "Pin important chats and group long-running work into projects. Reversible deletions remain available from the recycle bin."))
